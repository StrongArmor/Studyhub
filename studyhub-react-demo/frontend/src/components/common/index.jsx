...
export function Navbar({ active }) {
  ...
  return <nav className="navbar"><div className="navbar-brand" onClick={() => navigate('home')}><div className="navbar-logo"><Logo /></div><span className="navbar-name">StudyHub</span></div><div className="navbar-links"><span className={`navbar-link ${active === 'tutors' ? 'active' : ''}`} onClick={() => navigate('tutors')}>🔍 Tìm gia sư</span><span className={`navbar-link ${active === 'become-tutor' ? 'active' : ''}`} onClick={() => navigate('become-tutor')}>📋 Trở thành gia sư</span>{!user ? <><span className={`navbar-link ${active === 'role-select' ? 'active' : ''}`} onClick={() => navigate('register')}>Đăng ký</span><span className={`navbar-link ${active === 'role-select' ? 'active' : ''}`} onClick={() => navigate('role-select')}>Đăng nhập</span></> : <div className="user-menu"><button className="user-trigger" onClick={() => setOpen((v) => !v)}><span className="user-avatar">{user.avatar || user.name.split(' ').map((p) => p[0]).join('').slice(0, 2).toUpperCase()}</span><span>{user.name}</span></button>{open ? <div className="user-dropdown">{isTutor && <><button onClick={() => { setOpen(false); navigate('tutor-profile'); }}>Hồ sơ gia sư</button><button onClick={() => { setOpen(false); navigate('dashboard'); }}>Lịch dạy học</button></>}{!isTutor && <button onClick={() => { setOpen(false); navigate('dashboard'); }}>Trang cá nhân</button>}<button onClick={() => { setOpen(false); navigate('wallet'); }}>Ví tiền</button><button onClick={() => { setOpen(false); doLogout(); }}>Đăng xuất</button></div> : null}</div>}</div></nav>;
}

export function Footer() {
  const { navigate } = useApp();
  return <footer className="footer"><div className="footer-grid"><div className="footer-col"><h4>StudyHub</h4><p>Nền tảng kết nối gia sư và học viên với các buổi học ngắn 30-45 phút, linh hoạt thời gian.</p></div><div className="footer-col"><h4>Dành cho học viên</h4><a onClick={() => navigate('tutors')}>Tìm gia sư</a><a onClick={() => navigate('dashboard')}>Lịch học của tôi</a><a>Cách thức hoạt động</a></div><div className="footer-col"><h4>Dành cho gia sư</h4><a onClick={() => navigate('become-tutor')}>Đăng ký dạy học</a><a>Chính sách thanh toán</a><a>Hướng dẫn sử dụng</a></div><div className="footer-col"><h4>Liên hệ</h4><p>Email: support@studyhub.vn</p><p>Hotline: 1900 xxxx</p><p>Giờ làm việc: 8:00 - 22:00</p></div></div><div className="footer-bottom"><span>© 2026 StudyHub. All rights reserved.</span><span className="footer-admin" onClick={() => navigate('login-admin')}>Admin</span></div></footer>;
}
...
export function InputField({ label, icon, value, onChange, placeholder, type = 'text', noIcon = false, hint }) { return <div className="input-group">{label && <label className="input-label">{label}</label>}<div className="input-wrap">{!noIcon && <span className="input-icon">{icon}</span>}<input className={`input-field${noIcon ? ' no-icon' : ''}`} type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} /></div>{hint && <div className="input-hint">{hint}</div>}</div>; }
